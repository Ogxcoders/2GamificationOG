...
func CompileEngineConfig(ctx context.Context, pool *db.Pool, projectID, envID string) (*CompiledConfig, error) {
	...
	buckets := map[string]*[]any{
		TypeRule:        ptrTo(cfg["rules"].([]any)),
		TypeChallenge:   ptrTo(cfg["challenges"].([]any)),
		TypeStreak:      ptrTo(cfg["streaks"].([]any)),
		TypeAchievement: ptrTo(cfg["achievements"].([]any)),
		TypeLevelTrack:  ptrTo(cfg["level_tracks"].([]any)),
		TypeCurrency:    ptrTo(cfg["currencies"].([]any)),
		TypeLeaderboard: ptrTo(cfg["leaderboards"].([]any)),
		TypeWorkflow:    ptrTo(cfg["workflows"].([]any)),
	}
...
func ptrTo[T any](s []T) *[]T { return &s }
