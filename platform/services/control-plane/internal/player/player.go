...
func GetFullState(ctx context.Context, pool *db.Pool, projectID, envID, userID string) (*FullState, error) {
        ...
        st.Challenges = queryMaps(ctx, pool, `
                SELECT jsonb_build_object('challenge_id', latest.challenge_id,
                       'name', COALESCE(o.name, latest.challenge_id),
                       'progress', latest.progress, 'target', latest.target, 'status', latest.status,
                       'window_key', latest.window_key, 'completions', latest.completions,
                       'updated_at', to_char(latest.updated_at,'YYYY-MM-DD"T"HH24:MI:SS"Z"'))
                FROM (
                ...
                LEFT JOIN objects o ON o.id = latest.challenge_id AND o.project_id = $1
                ORDER BY challenge_id`, projectID, envID, userID)

        st.Achievements = queryMaps(ctx, pool, `
                SELECT jsonb_build_object('achievement_id', a.achievement_id,
                       'name', COALESCE(o.name, a.achievement_id),
                       'unlocked_at', to_char(a.unlocked_at,'YYYY-MM-DD"T"HH24:MI:SS"Z"'))
                FROM achievement_unlocks a
                LEFT JOIN objects o ON o.id = a.achievement_id AND o.project_id = a.project_id
                WHERE a.project_id=$1 AND a.environment_id=$2 AND a.user_id=$3
                ORDER BY a.unlocked_at DESC`, projectID, envID, userID)
...
func GetLeaderboard(ctx context.Context, pool *db.Pool, projectID, envID, leaderboardID, windowKey, me string, limit int) (*LeaderboardPage, error) {
        ...
        // NOTE: queryMaps scans a single jsonb column — wrap columns in
        // jsonb_build_object (raw multi-column selects scanned as one value
        // silently returned empty lists).
        page.Top = queryMaps(ctx, pool, fmt.Sprintf(`
                SELECT jsonb_build_object('user_id', user_id, 'score', score,
                       'achieved_at', to_char(achieved_at,'YYYY-MM-DD"T"HH24:MI:SS"Z"'),
                       'rank', %s, 'tied_with', %s)
                FROM leaderboard_entries e
