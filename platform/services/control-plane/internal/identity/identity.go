...
func UpsertUser(ctx context.Context, pool *db.Pool, projectID, envID, userID string, anonymous bool, displayName string, attrs map[string]any) (*User, error) {
        ...
        err := pool.QueryRow(ctx, `
                ...
                VALUES ($1, $2, $3, $4, $5, $6)
                ON CONFLICT (project_id, environment_id, id) DO UPDATE SET
...
func MergeAnonymous(ctx context.Context, pool *db.Pool, projectID, envID, sourceUserID, targetUserID string) error {
        ...
        simpleTables := []string{
                ...
                // NOTE: simulation_runs has no user_id column (project-scoped).
        }
        for _, t := range simpleTables {
                // NOTE: UPDATE ... ON CONFLICT is invalid PostgreSQL (ON CONFLICT is
                // INSERT-only). If the target already owns a row with the same unique
                // key, the move for that table hits unique_violation — acceptable:
                // the source user is archived anyway, and target keeps its state.
                if _, err := tx.Exec(ctx, `
