...
func (s *Server) AnalyticsOverview(w http.ResponseWriter, r *http.Request) {
        ...
        // Events by day. NOTE: ($3 || ' days')::interval fails (int || text has
        // no operator) — use make_interval instead.
        rows, err := s.Pool.Query(r.Context(), `
                SELECT to_char(date_trunc('day', occurred_at), 'YYYY-MM-DD') AS day, COUNT(*)
                FROM events
                WHERE project_id=$1 AND environment_id=$2 AND occurred_at > now() - make_interval(days => $3)
        ...
        // Top rules fired (from traces' matched rules). SRFs are illegal in WHERE —
        // expand nodes via an implicit LATERAL join instead.
        rows2, err := s.Pool.Query(r.Context(), `
                SELECT n->>'label' AS label, COUNT(*) AS fires
                FROM decision_traces t, jsonb_array_elements(t.nodes) AS n
                WHERE t.project_id=$1 AND t.environment_id=$2
                  AND t.created_at > now() - interval '7 days'
                  AND n->>'kind' = 'rule_matched'
